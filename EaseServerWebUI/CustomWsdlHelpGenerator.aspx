...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    int maxObjectGraphDepth = 10; // !!! В стандарте было 4, но этого не хватает для некоторых типов
    ...
    TypeItems GetTypeItems(XmlSchemaComplexType type) {
        ...
        if (type.ContentModel != null) {
            ...
            if (content is XmlSchemaComplexContentExtension) {
                ...
                // !!! Добавленно для наследования элементов
                
                items.Items = GetTypeItems((XmlSchemaComplexType)type.BaseXmlSchemaType).Items;
                TypeItems typeItems = GetTypeItems((XmlSchemaComplexType)type.BaseXmlSchemaType);
                foreach (XmlSchemaObject obj in typeItems.Attributes)
                {
                    if (!items.Attributes.Contains(obj))
                        items.Attributes.Add(obj);
                }
            }
        ...
        if (particle != null) {
            ...
            if (particle is XmlSchemaGroupRef) {
                ...
                foreach (XmlSchemaObject obj in ((XmlSchemaGroupBase)particle).Items)
                {
                    items.Items.Add(obj);
                }
    ...
    string GetRealPostUrl(string svrUrl)
    {
        if (svrUrl.StartsWith("http", StringComparison.InvariantCultureIgnoreCase))
        {
            int idx = svrUrl.IndexOf('/', 7);
            if (idx != -1)
            {
                return svrUrl.Substring(idx);
            }
            else
            {
                return "/";
  ...
  <body>
    ...
    <div id="content">
      ...
      <span visible='<%#ShowingMethodList && ServiceDocumentation.Length > 0%>' runat="server">
          <p class="intro"><%#ServiceDocumentation%><!--<a href='http://wiki.w1.ru/<%#ServiceName%>'>Подробнее...</a>--></p>
      </span>
            ...
            <itemtemplate>
              <li>
                ...
                <span visible='<%#((string)DataBinder.Eval(Container.DataItem, "Value.Documentation")).Length>0%>' runat=server>
                  <br><%#DataBinder.Eval(Container.DataItem, "Value.Documentation")%><!--<a href='http://wiki.w1.ru/<%#ServiceName%>: <%#EscapeParam(DataBinder.Eval(Container.DataItem, "Key").ToString())%>'>Подробнее...</a>-->
                </span>
              </li>
              <p>
            </itemtemplate>
          ...
          <% if (!showPost) { 
              ...
              <% }
                  ...
                  <% }
                      ...
                      <form target="_blank" action='<%#TryGetUrl == null ? "" : GetRealPostUrl(TryGetUrl.AbsoluteUri)%>' method="GET">
                 ...
                 <% }
                  ...
                  <% }
                      ...
                      <form target="_blank" action='<%#TryPostUrl == null ? "" : GetRealPostUrl(TryPostUrl.AbsoluteUri)%>' method="POST">                      
